...
export default function PacmanGame({ onWin }) {
  ...
  useEffect(() => {
    ...
    const onLoad = () => {
      setEnemyGifReady(true);
    };

    const onError = () => {
      setEnemyGifReady(false);
    };

    img.addEventListener("load", onLoad);
    img.addEventListener("error", onError);

    // 👇 SRC ALTIJD ALS LAATSTE
    img.src = "/images/diva_talking3.gif";

    enemyGifRef.current = img;

    return () => {
      img.removeEventListener("load", onLoad);
      img.removeEventListener("error", onError);
      enemyGifRef.current = null;
    };
  }, []);
  ...
  const draw = (ctx, s) => {
    ...
    const img = enemyGifRef.current;

    // 🔥 schaalfactor (1.0 = tile size)
    const DIVA_SCALE = 5;

    const size = cell * DIVA_SCALE;

    // centreer diva op de tile
    ...
    if (img && enemyGifReady) {
      img.decode?.(); // <-- dit is de magie
      ctx.drawImage(img, ex, ey, size, size);
    }
